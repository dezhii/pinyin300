...
export function GamePage({ level, onBack, onComplete, onStartLevel, onWrong }: GamePageProps) {
  ...
  if (result) {
    ...
    return (
      <section className="result-page panel">
        ...
        <h1>{result.wrongCharIds.length === 0 ? '满星到站！' : '到站啦，继续加油！'}</h1>
        <div className="result-stars" aria-label={`${result.stars ?? 0} 颗星`}>
          {renderStars(result.stars ?? 0)}
        </div>
        <p>
          本关答对 {result.correct} 个汉字，尝试 {result.attempts} 次，最好连续答对 {result.bestStreak} 个。
        </p>
        ...
        {result.wrongCharIds.length > 0 && <p className="warm-note">答错的汉字已经放进错题本，可以稍后集中复习。</p>}

        <div className="result-actions">
          <button className="secondary-button" onClick={() => onStartLevel(level)} type="button">
            再玩一次
          </button>
          {!isLastLevel && (
            <button className="primary-button" onClick={() => onStartLevel(level + 1)} type="button">
              下一关
            </button>
          )}
